...
<body>
	<fieldset>
		...
		<ul>
			...
			<li>속성은 존재하나,getter가 없는 경우(에러):\${member.id}</li>
			<li>EL변수명이 틀린경우(에러 NO,출력NO): ${membar.id }</li>
		</ul>
		...
		존재하지 않은 속성이나 키값으로 접근시 null임으로 에러는 안나고 출력만 안됨
  	 	...
  	 	\${null } : ${null }
  	 	<%
  	 	request.setAttribute("myreuqets", "리퀘스트 영역");
  	 	Map map = new HashMap();
  	 	map.put("mymap","맵 컬렉션");
  	 	%>
  	 	<c:set var="map" value="<%=map %>"/>
  	 	<ul>
  	 		<li>\${requestScope.myreuqets} : ${requestScope.myreuqets}</li>
  	 		<li>\${requestScope.yourreuqets} : ${requestScope.yourreuqets}</li>
  	 	...
  	 	\${100/0 } : ${100/0 }
  	 	<h2>임의의 변수로 .을 통해서 접근시-에러 NO,출력NO</h2>
  	 	\${myvar }:${myvar }<br/>
  	 	\${myvar.num }:${myvar.num }
	</fieldset>
</body>

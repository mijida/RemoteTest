...
<body>
	<div class="wrap">
		...
		<div class="content">
			...
			<div class="main">
				<h2>
					회원가입 완료(<%=getDate("yyyy-MM-dd")%>)
				</h2>
				<table	style="width: 100%; border-spacing: 1px; background-color: green">
					...
					<tr style="background-color: white">
						<td>성별</td>
						<td>
							<input type="radio" name="gender" value="man" <%if("MAN".equals(gender.toUpperCase())){ %>checked<% } %>/>남 
							<input type="radio" name="gender" value="woman" <%if("WOMAN".equals(gender.toUpperCase())){ %>checked<% } %>/>녀 
							<input type="radio"	name="gender" value="trans" <%if("TRANS".equals(gender.toUpperCase())){ %>checked<% } %>/>트랜스젠더
						</td>
					</tr>
					<tr style="background-color: white">
						<td>관심사항</td>

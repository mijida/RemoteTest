...
<body>
	<fieldset>
	    ...
	    <h4>문]todayString에서 Today는 Tomorrow로 03일은 04로 replace하여라</h4>
	    <c:set var="temp" value="${fn:replace(todayString,'Today','Tomorrow') }"/>
	    ${fn:replace(temp,"03","04") }<br/>	    
	    ${fn:replace(fn:replace(todayString,"Today","Tomorrow"),"03","04") }
	    <h2>String trim(String):양쪽의 공백 제거</h2>
	    X\${"     J   S  T L    "}Y[트림 적용 전] : X${"     J   S  T L    "}Y<br/>
	    X\${fn:trim("     J   S  T L    ")}Y[트림 적용 후] : X${fn:trim("     J   S  T L    ")}Y
	    ...
	    <h2>String join(String[],str):모든 배열요소를  str로 연결해서 하나의 문자열 반환</h2>
	    ${fn:join(array,"▲") }
	    <h2>int indexOf(str1,str2):str1에서 str2문자열의 시작 인덱스 반환</h2>

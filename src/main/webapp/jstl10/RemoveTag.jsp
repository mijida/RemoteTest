...
<body>
	<fieldset>
		...
		<my:set var="applicationvar" value="어플리케이션" scope="application"/>
		<h2>삭제 전 출력</h2>
		<ul>
		...
		<h2>속성명은 존재하나 영역이 틀린 경우(에러NO,삭제 NO)</h2>
		<my:remove var="requestvar" scope="session"/>
		<h2>영역이 틀린 속성을 삭제 후 출력</h2>
		<ul>
			<li>pagevar :${pagevar }</li>
			<li>pagevar :${requestScope.pagevar }</li>
			<li>requestvar:${requestvar }</li>
		...
		<h2>동일한 속성명이 여러 영역에 존재하는 경우(scope미 지정시:모두 삭제됨) 삭제 후 출력</h2>
